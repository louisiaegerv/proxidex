...
export function StorageNotice({ className = "", variant = "banner" }: StorageNoticeProps) {
  const isPro = useIsPro()
  const { showStorageNotice } = useStorageNotice()
  const dismissStorageNotice = useProxyList((state) => state.dismissStorageNotice)
  ...
  if (variant === "compact") {
    return (
      <div className={`rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 ${className}`}>
      ...
      <div className={`rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 ${className}`}>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="font-medium text-amber-200">Your decks are temporary</p>
            <p className="mt-1 text-sm text-amber-200/80">
              Free accounts use session storage. Your decks will be cleared when you close this tab.
            </p>
            <Link 
              href="/upgrade"
  ...
  return (
    <div className={`relative overflow-hidden rounded-lg border border-amber-500/30 bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-amber-500/10 p-4 ${className}`}>
      {/* Animated background effect */}
      <div className="absolute inset-0 bg-gradient-to-r from-amber-500/0 via-amber-400/5 to-amber-500/0 animate-pulse" />
      ...
      <div className="relative flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm text-amber-200/70">
            Free accounts use temporary storage. Your data will be cleared when you close this browser tab.
          </p>
