...
function getFullImageUrl(baseUrl: string | undefined, quality: 'low' | 'high' = 'high'): string | undefined {
...
export async function generateProxyPDF(
  ...
  const gridWidthMm = settings.cardsPerRow * imageWidthMm + (settings.cardsPerRow - 1) * settings.gap;
  const gridHeightMm = settings.rowsPerPage * imageHeightMm + (settings.rowsPerPage - 1) * settings.gap;
  ...
  if (bleedMm > 0) {
    const uniqueCards = [...new Map(cardsToPrint.map(c => [c.cardId, c])).values()];
  ...
  for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
    ...
    const pageCards = cardsToPrint.slice(startCardIndex, startCardIndex + cardsPerPage);
    ...
    if (settings.showCutLines) {
      ...
      const cutColor = settings.cutLineColor 
        ? rgb(settings.cutLineColor.r / 255, settings.cutLineColor.g / 255, settings.cutLineColor.b / 255)
        : rgb(0.06, 0.73, 0.49);
      const lineWidth = 0.75;
      const extension = mmToPoints(3);
      ...
      for (let row = 0; row < settings.rowsPerPage; row++) {
        for (let col = 0; col < settings.cardsPerRow; col++) {
          ...
          const cardTopY = pageSize.height - marginY - row * spacingY - bleedPoints;
    ...
    for (let i = 0; i < pageCards.length; i++) {
      ...
      const imageY = pageSize.height - marginY - row * spacingY - imageHeightPoints;
      ...
      const imageUrl = cachedBleedImage || getFullImageUrl(card.image, settings.imageQuality);
...
export function downloadPDF(pdfBytes: Uint8Array, filename: string = 'proxymon-cards.pdf') {
